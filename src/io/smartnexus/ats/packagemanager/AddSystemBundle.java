...
public class AddSystemBundle {

	private static final Logger log = Logger.getLogger(AddSystemBundle.class.getName());
	ExtentTest logger;
	...
	public void TEST_152_addPackageGroup() throws Exception {
		...
		String response = nbi.addPackageGroups(pmId,"System Bundle", "sys","4.9.1");
	...
	public void TEST_00_getPackageTypeIdAPI() throws Exception {
		logger = extent.startTest(this.getClass().getSimpleName());
		typeId = nbi.getPackageTypeId("Firmware");
		Assert.assertTrue(typeId.equals("17f7187b-51ff-4ddb-af18-31d8a007f256"));
	...
	public void TEST_00_addPackageImage() throws Exception {
		logger = extent.startTest(this.getClass().getSimpleName());
		imageId = nbi.addPackageImage(typeId);
	...
	public void TEST_406_addPackage() throws Exception {
		logger = extent.startTest(this.getClass().getSimpleName());
		String response = nbi.addPackage(pmId, packageGroupId, typeId, imageId);
		String[] s1 = response.split(",");
		int status = Integer.parseInt(s1[0]);
		packageId = s1[1];
		Assert.assertTrue(status >= 200 & status < 205);
	...
	public void TEST_00_getPackageBundle() throws Exception {
		logger = extent.startTest(this.getClass().getSimpleName());
